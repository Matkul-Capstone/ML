...
def analyze_pronunciation(reference, transcribed, window=3):
        ...
        for j in range(start, end):
            if j < len(trans_words):
                ...
                if ref_word.lower() == trans_word.lower():
                    correct_count += 1
                    found_match = True
                    correct_words.append(ref_word)  # Add to correct words list
                    break
    ...
    accuracy = int(round((correct_count / total_count) * 100))
    return accuracy, feedback, wrong_words, correct_words
...
async def transcribe_audio(file: UploadFile = File(...), reference_passage: str = Form(...)):
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            accuracy, feedback, wrong_words, correct_words = analyze_pronunciation(reference_passage, transcribed_text)
